...
void XBee::readPacket() {
    ...
    // timing profilling, C. Pham
    startParseMessage=millis();
    // we don't use these variables for the moment;
    startTxStatusResponse = 0;
    stopTxStatusResponse = 0;
    ...
    while (available()) {
        ...
        if (_pos > 0 && b == START_BYTE && ATAP == 2) {
        	...
        	_response.setErrorCode(UNEXPECTED_START_BYTE);
                // timing profilling, C. Pham
                stopParseMessage=millis();
                stopFetchedTime=millis();
        	return;
        }
